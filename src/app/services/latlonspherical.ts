...
const toRadians = function(n: number) { return n * Math.PI / 180; };
const toDegrees = function(n: number) { return n * 180 / Math.PI; };
...
class LatLonSpherical {
   ...
   */
  static parse(...args: any[]): LatLonSpherical {
   ...
   */
  distanceTo(point: LatLonSpherical, radius=6371e3): number {
    ...
    const R = radius;
    const φ1 = toRadians(this.lat),  λ1 = toRadians(this.lon);
    const φ2 = toRadians(point.lat), λ2 = toRadians(point.lon);
    const Δφ = φ2 - φ1;
    const Δλ = λ2 - λ1;
  ...
  initialBearingTo(point: LatLonSpherical): number {
    ...
    const φ2 = toRadians(point.lat);
    const Δλ = toRadians((point.lon - this.lon));
   ...
   */
  destinationPoint(distance: number, bearing: number, radius=6371e3): LatLonSpherical {
  ...
  static intersection(p1: LatLonSpherical, brng1: number, p2: LatLonSpherical, brng2: number): LatLonSpherical | null {
    ...
    const φ1 = toRadians(p1.lat), λ1 = toRadians(p1.lon);
    const φ2 = toRadians(p2.lat), λ2 = toRadians(p2.lon);
    const θ13 = toRadians(Number(brng1)), θ23 = toRadians(Number(brng2));
   ...
   */
  crossTrackDistanceTo(pathStart: LatLonSpherical, pathEnd: LatLonSpherical, radius=6371e3): number {
    ...
    const θ13 = toRadians(pathStart.initialBearingTo(this));
    const θ12 = toRadians(pathStart.initialBearingTo(pathEnd));
   ...
   */
  alongTrackDistanceTo(pathStart: LatLonSpherical, pathEnd: LatLonSpherical, radius=6371e3): number {
    if (!(pathStart instanceof LatLonSpherical)) pathStart = LatLonSpherical.parse(pathStart); // allow literal forms
    if (!(pathEnd instanceof LatLonSpherical)) pathEnd = LatLonSpherical.parse(pathEnd);       // allow literal forms
    const R = radius;
   ...
   */
  static crossingParallels(point1: LatLonSpherical, point2: LatLonSpherical, latitude: number): { lon1: number, lon2:number } | null {
    if (point1.equals(point2)) return null; // coincident points

    const φ = toRadians(Number(latitude));
    ...
    const lon1 = toDegrees(λi1);
    const lon2 = toDegrees(λi2);
   ...
   */
  rhumbDistanceTo(point: LatLonSpherical, radius=6371e3): number {
    if (!(point instanceof LatLonSpherical)) point = LatLonSpherical.parse(point); // allow literal forms
    ...
    const φ2 = toRadians(point.lat);
    const Δφ = φ2 - φ1;
    let Δλ = toRadians(Math.abs(point.lon - this.lon));
  ...
  rhumbBearingTo(point: LatLonSpherical): number {
    ...
    const φ1 = toRadians(this.lat);
    const φ2 = toRadians(point.lat);
    let Δλ = toRadians((point.lon - this.lon));
    // if dLon over 180° take shorter rhumb line across the anti-meridian:
    if (Math.abs(Δλ) > π) Δλ = Δλ > 0 ? -(2 * π - Δλ) : (2 * π + Δλ);
   ...
   */
  rhumbDestinationPoint(distance: number, bearing: number, radius=6371e3): LatLonSpherical {
    const φ1 = toRadians(this.lat), λ1 = toRadians(this.lon);
    const θ = toRadians(Number(bearing));
  ...
  rhumbMidpointTo(point: LatLonSpherical): LatLonSpherical {
    ...
    const φ1 = toRadians(this.lat); let λ1 = toRadians(this.lon);
    const φ2 = toRadians(point.lat), λ2 = toRadians(point.lon);
   ...
   */
  static areaOf(polygon: LatLonSpherical[], radius=6371e3): number {
    ...
    for (let v=0; v<nVertices; v++) {
      const φ1 = toRadians(polygon[v].lat);
      const φ2 = toRadians(polygon[v+1].lat);
      const Δλ = toRadians((polygon[v+1].lon - polygon[v].lon));
   ...
   */
  toString(format='d', dp=undefined): string {
