...
class Dms {
    ...
    static parse(dms: string | number): number {
        ...
        if (!isNaN(parseFloat(dms.toString())) && isFinite(dms as number)) return Number(dms);

        // strip off any sign or compass dir'n & split out separate d/m/s
        const dmsParts: string[] = String(dms).trim().replace(/^-/, '').replace(/[NSEW]$/i, '').split(/[^0-9.,]+/);
        ...
        switch (dmsParts.length) {
            case 3:  // interpret 3-part result as d/m/s
                deg = parseFloat(dmsParts[0])/1 + parseFloat(dmsParts[1])/60 + parseFloat(dmsParts[2])/3600;
                break;
            case 2:  // interpret 2-part result as d/m
                deg = parseFloat(dmsParts[0])/1 + parseFloat(dmsParts[1])/60;
                break;
            case 1:  // just d (possibly decimal) or non-separated dddmmss
                deg = parseFloat(dmsParts[0]);
     ...
     */
    static toDms(deg: number, format='d', dp=undefined): string {
        if (isNaN(deg)) return null;  // give up here if we can't make a number from deg
        if (typeof deg === 'string' && (deg as string).trim() == '') return null;
     ...
     */
    static compassPoint(bearing: number, precision=3): string {
